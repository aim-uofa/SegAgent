...
class Lvis_v1_Dataset(ISDataset):
    ...
    def get_sample(self, index) -> DSample:
        ...
        for indx, obj_annotation in enumerate(image_annotations):
            ...
            if self.min_area_thres > object_area:
                continue
            #print(object_area)
            caption = self.id2class[obj_annotation['category_id']]
            ...
            objects_ids.append((ann_id,1))
            instance_to_class[(ann_id,1)] = caption
            ann_id += 1
            gt_masks.append(mask)
        if len(gt_masks) == 0:
            return None
        gt_masks= np.array(gt_masks).transpose(1,2,0)
        assert ann_id == gt_masks.shape[2]
        return DSample(image, gt_masks, objects_ids=objects_ids, instance_to_class=instance_to_class)
...
if __name__ == '__main__':
    dataset = Lvis_v1_Dataset('data/cocolvis', split='val')
